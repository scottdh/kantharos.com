	...
	class TGM_Plugin_Activation {
		...
		protected function do_plugin_install() {
			...
			if (isset($_GET[sanitize_key('plugin')]) && (isset($_GET[sanitize_key('tgmpa-install')]) && 'install-plugin' == $_GET[sanitize_key('tgmpa-install')])) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'ct_theme' ) . '">' . __( 'Return to the Dashboard', 'ct_theme' ) . '</a>' ) . '</p>';
		...
		public function notices() {
			...
			if (!get_user_meta(get_current_user_id(), 'tgmpa_dismissed_notice', true)) {
                ...
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'ct_theme' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'ct_theme' ) . '</a>'  : '';
                ...
                $action_links = apply_filters(
                    ...
                    array(
                        ...
                        'dismiss'  => '<a class="dismiss-notice" href="' . esc_url( add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) ) . '" target="_parent">' . __('Dismiss this notice', 'ct_theme') . '</a>',
                    )
                );
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function column_plugin($item) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'ct_theme' ) . ' %2$s">' . __( 'Install', 'ct_theme' ) . '</a>',
                        esc_url(
						    wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'ct_theme' ) . ' %2$s">' . __( 'Activate', 'ct_theme' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
		...
		public function process_bulk_actions() {
			...
			if ('tgmpa-bulk-install' === $this->current_action()) {
                ...
                $url   = esc_url_raw( add_query_arg( array( 'page' => urlencode( TGM_Plugin_Activation::$instance->menu ) ), admin_url( 'themes.php' ) ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
	...
	if (!class_exists('TGM_Bulk_Installer_Skin')) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'ct_theme' ) . '">' . __( 'Return to the Dashboard', 'ct_theme' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
