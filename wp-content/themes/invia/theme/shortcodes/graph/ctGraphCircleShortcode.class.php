...
class ctGraphCircleShortcode extends ctShortcode {
	...
	public function handle($atts, $content = null) {
		...
		if ($icon) {
			$text = '<span' . $verStyle . ' class="entypo ' . $icon . '"><i></i></span>';
			if (!$hasFontSize) {
				$fontsize *= 2;
			}
		}
        ...
        $cParams=array(
            'style'=>($fontcolor ?'color:'.esc_attr($fontcolor).';':'').'font-size:'. esc_attr($fontsize).'px',
            'class'=>array('graph-circle-'.($icon ? 'icon' : 'text'),'graph-circle-size-'.$size,'graph-circle'),
            'data-percent'=>$percent,
            'data-barcolor'=>esc_attr($barcolor),
            'data-trackcolor'=>esc_attr($trackcolor),
            'data-scalecolor'=>esc_attr($scalecolor),
            'data-linecap'=>esc_attr($linecap),
            'data-linewidth'=>esc_attr($linewidth),
            'data-size'=>esc_attr($size),
            'data-animate'=>esc_attr($animate)
        );
		return '<div'.$this->buildContainerAttributes($cParams,$atts).'>' . $text . '</div>';
	}
	...
	public function getAttributes() {
		return array(
			...
			'text' => array('label' => __("text inside circle", 'ct_theme'), 'help' => __('if icon selected, text will not be displayed')),
			'icon' => array('label' => __('entypo icon', 'ct_theme'), 'type' => "icon", 'default' => '', 'link' => CT_THEME_ASSETS . '/shortcode/entypo/index.html'),
			'barcolor' => array('label' => __('bar color', 'ct_theme'), 'default' => '#ff6138', 'type' => 'colorpicker', 'help' => __('The color of the curcular bar.', 'ct_theme')),
			'trackcolor' => array('label' => __('track color', 'ct_theme'), 'default' => '#d7d7d7', 'type' => 'colorpicker', 'help' => __('The color of the track for the bar, leave it blank to disable rendering.', 'ct_theme')),
			...
			'vertposition' => array('label' => "vertical position", 'default' => '0', 'help' => array('useful for positioning icons. Negative value will move content up, position down. Value in px.')),
			'animate' => array('label' => __('animation speed', 'ct_theme'), 'default' => '1000', 'help' => __('Time in milliseconds (1000 is 1 second) for a eased animation of the bar growing, or 0 to deactivate.')),
		);
	}
}
