...
class ctPortfolioType extends ctPortfolioTypeBase {
	...
	public function addMetaBox() {
		...
		ctSimpleMetaTermExtension::getInstance()->connectMeta('portfolio');
	}
	...
	public function portfolioMeta() {
		...
		$client_icon = isset($custom["client_icon"][0]) ? ($custom["client_icon"][0] ? $custom["client_icon"][0] : "user") : '';
		$date = isset($custom["date"][0]) ? $custom["date"][0] : "";
		$date_icon = isset($custom["date_icon"][0]) ? ($custom["date_icon"][0] ? $custom["date_icon"][0] : "calendar") : "";
		$tools = isset($custom["tools"][0]) ? $custom["tools"][0] : "";
		$tools_icon = isset($custom["tools_icon"][0]) ? ($custom["tools_icon"][0] ? $custom["tools_icon"][0] : "tools") : "";
		...
		if ($supportsRevolutionSlider = function_exists('rev_slider_shortcode')) {
			global $wpdb;
			$slides = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."revslider_sliders");
		}
		?>
		<p>
		...
		<p>
			...
			<?php $clientIcons = sprintf(__(" View %s and enter icon name ex. user", 'ct_theme'), '<a target="_blank" id="open_client_icon' . '" href="' . CT_THEME_ASSETS . '/shortcode/entypo/index.html' . '">' . __('available icons', 'ct_theme') . '</a>')?>
		...
		<p>
			...
			<?php $dateIcons = sprintf(__(" View %s and enter icon name ex. user", 'ct_theme'), '<a target="_blank" id="open_date_icon' . '" href="' . CT_THEME_ASSETS . '/shortcode/entypo/index.html' . '">' . __('available icons', 'ct_theme') . '</a>')?>
		<p class="howto"><?php echo __("Link to icon representation of the date field", 'ct_theme') . $dateIcons?></p>

		<p>
			<label for="tools"><?php _e('Tools', 'ct_theme')?>: </label>
			<input id="tools" class="regular-text" name="tools" value="<?php echo $tools; ?>"/>
		<p class="howto"><?php _e("Information about tools", 'ct_theme')?></p>
		<p>
			<label for="tools_icon"><?php _e('Tools icon', 'ct_theme')?>: </label>
			<input id="tools_icon" class="regular-text" name="tools_icon" value="<?php echo $tools_icon; ?>"/>
			<?php $toolsIcons = sprintf(__(" View %s and enter icon name ex. user", 'ct_theme'), '<a target="_blank" id="open_tools_icon' . '" href="' . CT_THEME_ASSETS . '/shortcode/entypo/index.html' . '">' . __('available icons', 'ct_theme') . '</a>')?>
		<p class="howto"><?php echo __("Link to icon representation of the tools field", 'ct_theme') . $toolsIcons?></p>
	...
	public function saveDetails() {
		...
		$fields = array('client_icon', 'date', 'date_icon', 'tools', 'tools_icon', 'external_label', 'show_title', 'subtitle', 'show_breadcrumbs', 'use_boxed');
